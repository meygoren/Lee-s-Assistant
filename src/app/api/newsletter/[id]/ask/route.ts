...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [entry, settings] = await Promise.all([
    prisma.newsletterEntry.findUnique({ where: { id } }),
  ...
  const systemPrompt = `Lee is asking a follow-up question about this AI newsletter digest he just read. Answer clearly and simply — he's a beginner with AI. Keep it focused and concise (a few sentences, more only if genuinely needed).
...
${priorQA ? `\nPrior follow-up questions on this digest:\n${priorQA}\n` : ""}`;
  ...
  const updated = await prisma.newsletterEntry.update({ where: { id }, data: { followUps } });
